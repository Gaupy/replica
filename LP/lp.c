...
/*		printf("No results for size=%d_serv=%d_iter=%d \n", t, s, iter);*/
	...
	fscanf(fp,"%d",&K);
	fscanf(fp,"%d",&T);
	fscanf(fp,"%d",&S); S = T; /*The number of servers is not important anymore. */
...
/* If you have access to cplex, use this to solve the problem*/
	sprintf(name, "pbm_size=%d_serv=%d_expe=%d_iter=%d_general.lp", t, s, l, iter);
...
/*We now study the solution for the different heuristics */
...
/*We now create the solution for the move 2 allocation */
	float verif_move2;
	if ( expe_number == 1 ) {
...
/*This file is where we write the results obtained with the caml procedure and the verifications.*/
        sprintf(name, "result_%d_%d_%d_%d.temp", t, s, l, expe_number);
	fp2 = fopen(name,"w");
	if (fp2 == NULL) {
